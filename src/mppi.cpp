...
Eigen::Vector2d MPPI::control(Eigen::Vector4d goal_state, const double acceleration = 0.0){
    ...
    Eigen::Vector2d u;
    Eigen::Matrix2d du;
    Eigen::Vector4d current_state = m_current_state_buf.back();
    m_current_state_buf.clear();
    for(int i = 0; i < m_mppiParams.number_rollouts; i++){
        mppi::Path newPath(m_pathParams, goal_state, current_state, acceleration);
...
void MPPI::registerCurrentState(Eigen::Vector4d current_state){
    m_current_state_buf.push_back(current_state);
}
